...
export const fisioEliteApiService = {
  ...
  async login(email: string, password: string): Promise<{ access_token: string; token_type: string; role: UserRole }> {
    const body = new URLSearchParams({ grant_type: 'password', username: email, password, scope: '', client_id: '', client_secret: '' });
